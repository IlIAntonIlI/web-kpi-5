import processedContentTypes from './content-types'
...
export class Router {
    ...
    async handle(request, response) {
        ...
        const path = new URL(url || '/', `http://${request.headers.host}`)
            .pathname
        ...
        if (request.headers.get('content-type')) {
            ...
            if (processedContentTypes[contentType]) {
                payload = await processedContentTypes[contentType](request)
            }
        }

        let body = {}
        for (const handler of this.handlers[path][method]) {
            const res = await handler(request, response, payload)
